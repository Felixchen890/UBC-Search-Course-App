...
export class QueryHandlerRoom {
	...
	private roomKeyArray: any;

	constructor(query: any, reportSet: InsightDataset[], data: CachedDataRoom[], id: any) {
		// dummy constructor
		this.query = query;
		...
		this.roomKeyArray = ["fullname","shortname","number","name","address","lat","lon","seats","type"
			,"furniture","href","setID"];
	}

	// translate CachedDataSection into InsightResult that can be more easily referred later
	private setTranslater(data: CachedDataRoom[]): InsightResult[]{
		let result = [];
...
// modifies: the current helper's columns aspect, keeps track of what kind of data should we include in the result
	...
	private transformationResultHandler(resultsList: InsightResult[]) {
		...
		for (let result of resultsList){
			let handledResult: InsightResult = {};
			for(const column of this.columns){
				if (this.roomKeyArray.includes(column)) {
					handledResult[`${this.setID}_${column}`] = result[column];
				} else {
	...
	private sortHandler(results: InsightResult[]): InsightResult[] {
		...
		} else if (orderKey === "fullname" || orderKey === "shortname" ||
			orderKey === "number" || orderKey === "name" || orderKey === "address"
			|| orderKey === "type" || orderKey === "furniture" || orderKey === "href") {
